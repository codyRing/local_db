CREATE or replace FUNCTION mint.insert_description()
    RETURNS text
    LANGUAGE plpgsql
...
where
seed.description <> hist.description
ON CONFLICT  DO NOTHING
;

	get diagnostics row_affects := row_count;
	return concat(row_affects::text,' :new description');
end;
$BODY$;

ALTER FUNCTION mint.insert_description()
    OWNER TO postgres;