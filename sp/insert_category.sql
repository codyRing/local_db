CREATE or replace FUNCTION mint.insert_category()
    RETURNS text
    LANGUAGE plpgsql
...
where
seed.category <> hist.category
ON CONFLICT  DO NOTHING
;

	get diagnostics row_affects := row_count;
	return concat(row_affects::text,' :new category');
end;
$BODY$;

ALTER FUNCTION mint.insert_category()
    OWNER TO postgres;